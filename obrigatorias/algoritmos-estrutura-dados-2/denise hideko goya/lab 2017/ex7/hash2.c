...
//faz o hash melhor
long int hash (tipoChave v, int m)
{
   if(m == 0)
      return 1;
      
   return (hash(v, m - 1) + 1) % m;
}
...
// Remove chave na tabela de símbolos.
//
void STremove(tipoChave v) 
{ 
   int h;
   h = hash(v, M);
...
//printf ("ra=%d hash=%d\n", obj.chave, h);
   ...
   //remove da lista
   tipoObjeto obj = STsearch(v);
   obj->chave = obj->next->chave;
